...
function cp_language_text($key)
{
    ...
    if (!isset($list))
    {
        $list = \BitrixHelperLib\Classes\Block\Getter::instance()
...
function cp_group_by_section($iblocks, $level = 0)
{
    ...
    $sections = \BitrixHelperLib\Classes\Section\Getter::instance()->setFilter(array(
        'ID' => $sectionsIds
...
function cp_get_iblock_dates($filter = array())
{
    ...
    \BitrixHelperLib\Classes\Block\Getter::instance()
        ->setOrder(array('DATE_ACTIVE_FROM' => 'DESC'))
        ->setFilter($filter)
        ->setHydrationMode(\BitrixHelperLib\Classes\Block\Getter::HYDRATION_MODE_ARRAY)
