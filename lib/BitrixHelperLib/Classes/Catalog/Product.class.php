...
class Product extends \BitrixHelperLib\Classes\Block\Object
{
    private $price = null;
    ...
    function getOffers()
    {
        ...
        if (is_array($arInfo))
        {
            $retval = \BitrixHelperLib\Classes\Block\Getter::instance()->setFilter(array(
                    'IBLOCK_ID' => $arInfo['IBLOCK_ID'],
                    'PROPERTY_' . $arInfo['SKU_PROPERTY_ID'] => $this->id
            ))
                ->setClassName('\BitrixHelperLib\Classes\Catalog\Offer')
                ->get();
        }
...
//            echo '<pre>'.print_r($this->price,true).'</pre>';
    ...
    protected function fillCatalogData()
    {
        ...
        {
            $ar = \BitrixHelperLib\Classes\Block\Getter::instance()
